...
instance DbOperation Stage where
    ...
    find  pool id = do 
                        res <- fetch pool (Only id) "SELECT id, recipeid, type, temp, time FROM stages WHERE id=?" :: IO [(Maybe Integer, Integer, Integer, Integer, Integer )]
                        return $ oneStage res
                            where oneStage ((id, recipeid, recipe_type, temp, time) : _) = Just $ Stage id recipeid recipe_type temp time
                                  oneStager _ = Nothing

    list  pool = do
                    res <- fetchSimple pool "SELECT id, type, name, file FROM sensors" :: IO [(Maybe Integer, Integer, Integer, Integer, Integer)]
